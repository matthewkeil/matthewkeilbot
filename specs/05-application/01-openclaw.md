...
| `openclaw_sandbox_mode` | `off` | Agent sandbox mode |
...
| `openclaw_anthropic_api_key` | (from vault) | Injected via `anthropic_api_key` vault variable |
| `openclaw_telegram_bot_token` | (from vault) | Injected via `telegram_bot_token` vault variable |
...
Orchestrates the task files in order: install, configure, systemd, tailscale_serve (conditional on `openclaw_tailscale_serve`), health_check.
...
- Config file (`config.json5`): deployed from template on first deploy only (`when: not config_file.stat.exists`), preserving any user modifications made after initial deploy.
- Env file (`env`): always updated from template on every run, as secrets may change. Triggers `Restart openclaw` handler.
...
Sets three environment variables for the service:

- `OPENCLAW_PORT` — the configured port
- `ANTHROPIC_API_KEY` — from vault
- `TELEGRAM_BOT_TOKEN` — from vault

Deployed to `~/.openclaw/env`, mode `0600`, owned by service user. Always updated on every run.
...
- Env file contains API keys (mode `0600`).
- All secret-touching tasks use `no_log: true`.
- Config file deployed only on first run (preserves user modifications).
...
- Config file only deployed on first run (`when: not openclaw_config_file.stat.exists`).
- Env file always updated (secrets may change).
